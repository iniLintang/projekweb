...
if (!empty($_POST['id_pencari_kerja']) && !empty($_POST['nama_keterampilan']) && !empty($_POST['sertifikat_url'])) {
    $id_pencari_kerja = $_POST['id_pencari_kerja'];
    $nama_keterampilan = $_POST['nama_keterampilan'];
    $sertifikat_url = $_POST['sertifikat_url'];
    ...
    if ($con) {
        // Memeriksa apakah id_pencari_kerja valid dengan mencocokkan di tabel pencari_kerja dan keterampilan
        $sql = "
        ...
        // Jika id_pencari_kerja ditemukan di tabel keterampilan
        if (mysqli_num_rows($res) != 0) {
            // Mengupdate informasi keterampilan di database
            $sqlUpdate = "
                UPDATE keterampilan SET 
            ...
            if (mysqli_num_rows($checkRes) != 0) {
                // Jika ID ditemukan di tabel pencari_kerja, tambahkan data baru ke keterampilan
                $sqlInsert = "
                    INSERT INTO keterampilan (id_pencari_kerja, nama_keterampilan, sertifikat_url)
                    VALUES ('".$id_pencari_kerja."', '".$nama_keterampilan."', '".$sertifikat_url."')
                ";
